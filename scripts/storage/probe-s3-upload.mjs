/**
 * ponytail: one-shot S3 upload probe — run on VPS with production env.
 * Usage: node scripts/storage/probe-s3-upload.mjs
 */
...
async function tryUpload(label, pathStyle, withAcl) {
  ...
  const apiUrl = forcePathStyle
    ? `${endpoint}/${bucket}/${key}`
    : `${endpoint}/${key}`;
  const pubHead = await fetch(apiUrl, { method: "HEAD" });
  console.log(`[${label}] anonymous HEAD ${apiUrl}: ${pubHead.status}`);
  return true;
}

console.log(
  `endpoint=${endpoint} bucket=${bucket} forcePathStyle=${forcePathStyle} aclMode=${aclMode}`,
);

const results = [];
results.push(await tryUpload("pathStyle=false+acl", false, true));
results.push(await tryUpload("pathStyle=true+acl", true, true));
results.push(await tryUpload("pathStyle=true+noAclHeader", true, false));

console.log("\nSummary:", results.map((ok, i) => `${i}:${ok}`).join(", "));
