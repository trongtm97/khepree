-- Idempotent: register Khepree TTS Batch AI catalog product + desktop OAuth client.
-- Matches khepree-tts-batch-ai: slug, client_id, protocol/callback.
...
WHERE NOT EXISTS (SELECT 1 FROM products WHERE slug = 'khepree-tts-batch-ai');
...
SET
  ...
  updated_at = now()
WHERE slug = 'khepree-tts-batch-ai';
...
SELECT 'tts_batch_ai.access', 'boolean', 'Base access for Khepree TTS Batch AI'
WHERE NOT EXISTS (SELECT 1 FROM features WHERE key = 'tts_batch_ai.access');
...
WHERE p.slug = 'khepree-tts-batch-ai' AND pl.slug = 'trial'
...
FROM plans pl
JOIN products p ON p.id = pl.product_id
JOIN features f ON f.key = 'tts_batch_ai.access'
WHERE p.slug = 'khepree-tts-batch-ai' AND pl.slug = 'trial'
  AND NOT EXISTS (
...
SET
  ...
  access_term_days = 1,
  status = 'active',
  internal_code = 'TTS_BATCH_AI_FREE_TRIAL',
  updated_at = now()
FROM products p
WHERE p.id = pl.product_id AND p.slug = 'khepree-tts-batch-ai' AND pl.slug = 'trial';
...
SET
  ...
  access_term_days = 30,
  status = 'active',
  internal_code = 'TTS_BATCH_AI_MONTHLY',
  updated_at = now()
FROM products p
WHERE p.id = pl.product_id AND p.slug = 'khepree-tts-batch-ai' AND pl.slug = 'month';
...
SET
  ...
  status = 'active',
  internal_code = 'TTS_BATCH_AI_YEARLY',
  updated_at = now()
FROM products p
WHERE p.id = pl.product_id AND p.slug = 'khepree-tts-batch-ai' AND pl.slug = 'year';

INSERT INTO plan_translations (plan_id, locale, name)
SELECT pl.id, 'vi', 'Tháng'
FROM plans pl
JOIN products p ON p.id = pl.product_id
WHERE p.slug = 'khepree-tts-batch-ai' AND pl.slug = 'month'
ON CONFLICT (plan_id, locale) DO UPDATE SET name = EXCLUDED.name, updated_at = now();
...
FROM plans pl
JOIN products p ON p.id = pl.product_id
WHERE p.slug = 'khepree-tts-batch-ai' AND pl.slug = 'year'
ON CONFLICT (plan_id, locale) DO UPDATE SET name = EXCLUDED.name, updated_at = now();
...
JOIN products p ON p.id = pl.product_id
JOIN features f ON f.key = 'tts_batch_ai.access'
WHERE p.slug = 'khepree-tts-batch-ai' AND pl.slug IN ('month', 'year')
  AND NOT EXISTS (
    SELECT 1 FROM plan_features pf WHERE pf.plan_id = pl.id AND pf.feature_id = f.id
  );
...
SELECT
  'khepree-tts-batch-ai-desktop',
  p.id,
  'Khepree TTS Batch AI',
  '["khepreettsbatchai://auth/callback"]'::jsonb,
  'active'
FROM products p
WHERE p.slug = 'khepree-tts-batch-ai'
  AND NOT EXISTS (SELECT 1 FROM desktop_clients WHERE client_id = 'khepree-tts-batch-ai-desktop');
...
WHERE dc.client_id = 'khepree-tts-batch-ai-desktop';
