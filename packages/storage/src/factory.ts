...
function createForBucket(bucket: StorageBucket): ObjectStorage {
  ...
  if (bucket === "public") {
    ...
    throw new StorageConfigurationError(
      "Public R2 bucket is not configured. Set R2_BUCKET_PUBLIC and credentials.",
    );
  }
  ...
  if (isDevLike()) return new MockObjectStorage();
  throw new StorageConfigurationError(
    "Private R2 bucket is not configured. Set R2_BUCKET_PRIVATE — never fall back to public.",
  );
}
