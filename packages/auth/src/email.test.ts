...
describe("auth email content", () => {
  it("builds verification email with url", () => {
    const content = verificationEmailContent("http://localhost:3001/verify?token=abc", "Alex", "en");
  ...
  it("builds reset password email", () => {
    const content = resetPasswordEmailContent("http://localhost:3001/reset?token=xyz", null, "en");
