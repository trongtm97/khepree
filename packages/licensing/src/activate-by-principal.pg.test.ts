...
describe.skipIf(!pg)("activateByPrincipal concurrency (Postgres)", () => {
  it("allows only one of two concurrent activations when devices.max is 1", async () => {
    ...
    if (!granted.license) throw new Error("expected license");
    ...
    const results = await Promise.allSettled([
      licensing.activateByPrincipal({
        principal: { type: "USER", id: userId },
        productId: product.id,
        installationId: `a-${suffix}`,
        platform: "windows",
      }),
      licensing.activateByPrincipal({
        principal: { type: "USER", id: userId },
        productId: product.id,
        installationId: `b-${suffix}`,
        platform: "macos",
      }),
    ]);
