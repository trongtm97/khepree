...
export async function POST(request: Request) {
  ...
  const limited = await enforceRateLimit(request, RATE_LIMITS.LICENSE, "activate");
  ...
  try {
    const result = await getPlatform().licensing.activate({
      licenseKey,
      installationId,
    ...
    return jsonError("INTERNAL", "Activation failed", 500, requestId);
  }
}
