...
export function AuthLayout({ children, copy }: { children: ReactNode; copy: AuthCopy }) {
  return (
    <div className="flex min-h-screen bg-khepree-cloud">
      <aside className="hidden lg:block lg:w-[min(44%,520px)] lg:shrink-0">
        <AuthBrandPanel copy={copy} />
      </aside>

      <div className="flex min-w-0 flex-1 flex-col">
        <header className="px-6 py-5 sm:px-8">
          <Link href={marketingPublicUrl()} target="_blank" rel="noopener noreferrer" className="inline-flex">
            <BrandLogo context="auth" />
          </Link>
        </header>

        <main className="flex flex-1 items-center justify-center px-4 pb-[max(2.5rem,env(safe-area-inset-bottom))] sm:px-8">
