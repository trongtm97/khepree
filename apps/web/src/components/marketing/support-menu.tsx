...
import { SUPPORT_NAV_KEYS, SUPPORT_NAV_LABEL_KEYS, SUPPORT_NAV_PATHS } from "./nav";
...
export function SupportMenu({ locale, messages }: { locale: SupportedLocale; messages: Messages }) {
  ...
  const active = SUPPORT_NAV_KEYS.some((key) => {
    const href = localePath(locale, SUPPORT_NAV_PATHS[key]);
    ...
    <div ref={rootRef} className="relative">
      ...
      {open ? (
        <div
          id={panelId}
          role="menu"
          aria-label={messages.nav.support}
        ...
        >
          <ul className="grid gap-0.5">
            {SUPPORT_NAV_KEYS.map((key) => (
              <li key={key}>
                <SupportLinkItem
                  href={localePath(locale, SUPPORT_NAV_PATHS[key])}
                  label={messages.nav[SUPPORT_NAV_LABEL_KEYS[key]]}
...
export function SupportMobileLinks({
  locale,
  messages,
  onNavigate,
}: {
  ...
  return (
    <div className="mt-4 border-t border-border pt-4">
      <p className="px-3 pb-2 text-xs font-semibold uppercase tracking-wide text-muted">{messages.nav.support}</p>
